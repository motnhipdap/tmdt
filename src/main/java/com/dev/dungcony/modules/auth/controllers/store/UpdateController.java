...
public class UpdateController {
    ...
    @Operation(
            summary = "Bước 1 — Gửi OTP xác nhận đổi email",
            description = """
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "OTP đã được gửi về email cũ",
                    content = @Content(examples = @ExampleObject(value = """
                            { "success": true, "message": "send otp req update email successfully", "data": null }
                            """))
            ),
            @ApiResponse(responseCode = "401", description = "Email không khớp với tài khoản đang đăng nhập",
                    content = @Content(examples = @ExampleObject(value = """
    ...
    public ResponseEntity<ApiRes<Void>> sendOtpChangeEmail(
            ...
            @Parameter(description = "Email hiện tại của tài khoản", required = true, example = "old@example.com")
    ...
    @Operation(
            summary = "Bước 2 — Xác nhận OTP và đổi sang email mới",
            description = """
    ...
    @ApiResponses({
            @ApiResponse(responseCode = "200", description = "Đổi email thành công",
                    content = @Content(examples = @ExampleObject(value = """
                            { "success": true, "message": "successfully", "data": null }
                            """))
            ),
            @ApiResponse(responseCode = "401", description = "OTP sai hoặc đã hết hạn",
                    content = @Content(examples = @ExampleObject(value = """
    ...
    public ResponseEntity<ApiRes<Void>> confirmEmailChange(
            @Parameter(hidden = true) @AuthenticationPrincipal AccountDetails detail,
            @Parameter(description = "Email mới muốn đổi sang", required = true, example = "new@example.com")
            @RequestParam @NotBlank @Email String newEmail,
            @Parameter(description = "Mã OTP 6 số nhận qua email cũ", required = true, example = "123456")
    ...
    @ApiResponses({
            ...
            @ApiResponse(responseCode = "401", description = "Mật khẩu cũ không đúng",
                    content = @Content(examples = @ExampleObject(value = """
                            { "success": false, "message": "Invalid username or password", "data": null }
                            """))
            ),
    ...
    public ResponseEntity<ApiRes<Void>> updatePass(
            @Parameter(hidden = true) @AuthenticationPrincipal AccountDetails detail,
            @Valid @org.springframework.web.bind.annotation.RequestBody UpdatePasswordReq req) {
