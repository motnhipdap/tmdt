...
public class AuthController {
        ...
        @Operation(summary = "Đăng nhập", description = """
                        ...
                        **Lưu ý:** Header `X-Device-Id` dùng để quản lý session theo thiết bị. FE tự sinh và lưu lại (ví dụ: UUID ngẫu nhiên lần đầu mở app).
                        """)
        @RequestBody(required = true, content = @Content(mediaType = "application/json", schema = @Schema(implementation = LoginReq.class), examples = @ExampleObject(name = "Ví dụ", value = """
                        {
                          "username": "dungcony",
                          "password": "12345678"
                        }
                        """)))
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Đăng nhập thành công", headers = @Header(name = "Set-Cookie", description = "refresh_token=<token>; HttpOnly; Secure"), content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
                                        {
                                          "success": true,
                        ...
                        @ApiResponse(responseCode = "401", description = "Sai username hoặc password", content = @Content(examples = @ExampleObject(value = """
                                        ...
                                        """))),
                        @ApiResponse(responseCode = "401", description = "Tài khoản chưa xác thực email", content = @Content(examples = @ExampleObject(value = """
        ...
        @Operation(summary = "Refresh token", description = """
                        ...
                        **Trả về:** access token mới.
                        """)
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Lấy access token mới thành công", content = @Content(mediaType = "application/json", examples = @ExampleObject(value = """
                                        {
                                          "success": true,
                                        ...
                                        """))),
                        @ApiResponse(responseCode = "401", description = "Refresh token không hợp lệ hoặc đã hết hạn", content = @Content(examples = @ExampleObject(value = """
        ...
        @Operation(summary = "Đăng xuất", description = """
                        ...
                        **Yêu cầu:**
                        ...
                        """)
        @ApiResponses({
                        @ApiResponse(responseCode = "200", description = "Đăng xuất thành công", content = @Content(examples = @ExampleObject(value = """
